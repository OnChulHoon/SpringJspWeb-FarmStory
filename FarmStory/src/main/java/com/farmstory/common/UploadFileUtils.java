...
*/
    ...
    public static String getRootPath(String fileName, HttpServletRequest request) {
        ...
        if (mediaType != null) {
        	...
        	if(folderName.equals("plant-info")) {
        		return request.getSession().getServletContext().getRealPath(rootPath + "/upload-image/plant-info"); // 이미지 파일 경로
        	}else if(folderName.equals("diary-info")) {
        		return request.getSession().getServletContext().getRealPath(rootPath + "/upload-image/diary-info"); // 이미지 파일 경로
        	}
        }
